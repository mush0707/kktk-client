...
async function fetchPage(){
  ...
  state.loading = true
  try{
    const res = await mfgApi.getOutputs(props.storageId,{
      limit: LIMIT,
      offset: state.offset,
...
async function setStatus(row:any, status:'approved'|'written_off'|'cancelled'){
  ...
  if (status==='approved') await mfgApi.approveOutput(props.storageId, row.id)
  else if (status==='written_off') await mfgApi.writeoffOutput(props.storageId, row.id)
  else await mfgApi.cancelOutput(props.storageId, row.id)
  row.status = status
}
const details = reactive({ open:false, entry:null as any })
async function openDetails(row:any){
  const full = await mfgApi.getOutputById(props.storageId, row.id, { include:'products.product' })
...
async function cancelProduct(entry:any, product:any){
  await mfgApi.cancelOutputProduct(props.storageId, entry.id, product.id)
