<template>
  <div id="add-edit-truck" tabindex="-1" aria-hidden="true"
      ...
      <div class="px-4">
        <form @submit="store" class="flex flex-col gap-y-2">
          <div class="flex flex-col gap-y-2">
            ...
            <input type="text" name="license_plate" v-model="form.license_plate"
                   placeholder="458XO55"
                   pattern="[0-9]{3}[A-Za-z]{2}[0-9]{2}"
                   maxlength="7"
...
export default {
  ...
  methods: {
    store() {
      ...
      api.post('trucks', {
        ...
        this.loading = false;
        toast.success(this.$t('truck_successfully_created'));
        this.modalObj.hide();
  ...
  mounted() {
    this.modalObj = new Modal(document.getElementById('add-edit-truck'), {
      placement: 'top-right',
      closable: true, backdropClasses: 'bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-40 modal-backdrop',
      onHide: () => {
        this.$parent.showAddEditTruckModal = false;
      },
    })
    this.modalObj.show();
  }
