<template>
  <div id="add-edit-product-type-modal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative overflow-y-auto bg-white w-full max-w-4xl ">
      <div class="flex items-center justify-between p-4 rounded-t dark:border-gray-600">
        ...
        <p class="text-xl">{{ this.type ? $t('edit_type') : $t('new_type') }}</p>
        <button type="button"
      ...
      <div class="px-4 pb-4">
        <form @submit="store" class="flex flex-col gap-y-2">
          ...
          <div class="flex items-center mb-4">
            <input v-model="form.industrial" id="industrial-checkbox" type="checkbox" value="" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
...
export default {
  ...
  methods: {
    ...
    store() {
      ...
      api.post(url, {
        ...
        this.loading = false;
        toast.success(this.$t('type_successfully_created'));
        this.modalObj.hide();
  ...
  mounted() {
    this.modalObj = new Modal(document.getElementById('add-edit-product-type-modal'), {
      placement: 'center',
      closable: true, backdropClasses: 'bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-40 modal-backdrop',
      onHide: () => {
        this.$parent.showAddEditProductType = false;
      },
    })
    this.modalObj.show();
    if(this.type) {
