  ...
  methods: {
    activateAll(activate_all, manual = true) {
      if (manual) {
        ...
        if (activate_all) {
          this.without_ids = this.storage_product_group.types.map((type) => {
            return type.id;
          })
        } else {
          this.with_ids = this.storage_product_group.types.map((type) => {
            return type.id;
          })
        }
      }
    ...
    async removeGroup(group, groupIndex) {
      if (confirm(this.$t('are_you_sure'))) {
        group.loading = true;
        let url = "storages/" + this.$route.params.id + "/product-groups/" + group.id;
        if (this.$route.params.department_id) {
          url = "storages/" + this.$route.params.id + "/departments/" + this.$route.params.department_id + "/product-groups/" + group.id
        }
    ...
    async existStorageGroupList() {
      let url = "storages/" + this.$route.params.id + "/product-groups";
      if (this.$route.params.department_id) {
        url = "storages/" + this.$route.params.id + "/departments/" + this.$route.params.department_id + "/product-groups"
      }
    ...
    async getProductStorage(group) {
      ...
      await api.get(url).then((response) => {
        this.storage_product_group = response.data.data
        if (this.storage_product_group) {
          this.activate_all = !!this.storage_product_group.checked;
    ...
    async groupList(search) {
      try {
        ...
        if(this.$route.params.department_id) {
          url = "storages/" + this.$route.params.id + "/product-groups"
        }
    ...
    async typeList() {
      ...
      if(this.$route.params.department_id) {
        url = "storages/"+this.$route.params.id+"/product-groups/"+group_id+"/types";
      }
      await api.get(url, {
        params: {
