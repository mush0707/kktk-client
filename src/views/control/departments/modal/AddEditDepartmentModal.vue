<template>
  <div id="add-edit-department-modal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative h-screen overflow-y-auto bg-white w-full max-w-4xl ">
      <div class="flex items-center justify-between p-4 rounded-t dark:border-gray-600">
        ...
        <p class="text-xl">{{ this.department ? $t('edit_company_department') : $t('new_company_department') }}</p>
        <button type="button"
...
export default {
  ...
  methods: {
    store() {
      ...
      api.post(url, {
        ...
        this.loading = false;
        toast.success(this.$t('department_successfully_created'));
        this.modalObj.hide();
  ...
  mounted() {
    this.modalObj = new Modal(document.getElementById('add-edit-department-modal'), {
      placement: 'top-right',
      closable: true, backdropClasses: 'bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-40 modal-backdrop',
      onHide: () => {
        this.$parent.showAddEditDepartment = false;
      },
    })
    this.modalObj.show();
    if(this.storage) {
